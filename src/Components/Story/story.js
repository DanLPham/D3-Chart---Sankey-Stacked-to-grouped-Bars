...
import { getStoryInfo } from './popout';
...
export function getStory(title, oWidth, hops) {
  ...
  axios
    ...
    .then(res => {
      ...
      if (res.data.error) {
        ...
        drawStory(res.data.videos, res.data.links, oWidth);
...
function drawHopSlider(title, oWidth, hops) {
  ...
  function changeNumberHops(e) {
    ...
    if (selected === 1 && hops !== 1) {
      getStory(title, oWidth, 1);
    } else if (selected === 2 && hops !== 2) {
      getStory(title, oWidth, 2);
    } else if (selected === 3 && hops !== 3) {
      getStory(title, oWidth, 3);
    }
  }
}

export function drawStory(nodesArrUnfiltered, linksArrUnfiltered, oWidth) {
